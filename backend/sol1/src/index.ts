...
export async function initializeAll() {
  ...
  Users will ask you questions that require you to retrieve data from a knowledge graph database, you have one tool available for this:
  1. "neo4jTool" for retreiving (generating and running queries) data from the database. 
  Only use this schema when generating and querying with the "neo4jTool":
  ${kgSchema}
  If the "neo4jTool" returns 0 or sum to zero, it is most likely that you will need to query a different table and calculate the total yourself. 
...
export async function handleUserQuestion(
  ...
  const finalState = await (
    ...
    {
      configurable: {
        thread_id: threadId || "ablation-study",
      },
    }
  );
